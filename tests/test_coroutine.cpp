...
int main() {
    ...
    // 顺序协程组合：建表 -> 插入 -> 查询 -> 清理
    auto work = [&](uORM::DataSource& d) -> asio::awaitable<int> {
        co_await uORM::executeCoro(d, "CREATE TABLE IF NOT EXISTS coro_t (id INTEGER PRIMARY KEY AUTOINCREMENT, v INT)");
        for (int i = 1; i <= 5; ++i) {
    ...
    {
        auto f = uORM::spawn(uORM::transactionCoro(ds, [&](uORM::IConnection& c) {
            uORM::executeUpdate(c, "INSERT INTO coro_t (v) VALUES (222)");
    ...
    // 并发协程：8 个同时查询
    {
        ...
        for (int i = 0; i < 8; ++i) {
            futs.push_back(uORM::spawn(uORM::queryCoro(ds, "SELECT ? + ?", {uORM::SqlValue(i), uORM::SqlValue(100)})));
        }
        bool allOk = true;
        for (int i = 0; i < 8; ++i) {
            auto r = futs[i].get();
